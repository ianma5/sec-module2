...
document.getElementById("textForm").addEventListener("submit", (event)=> {
    ...
    const regex = / {2,}/g; // 2 or more white spaces
    const newText = input.value.replaceAll(regex, " ").trim(); // trim gets rid of white space after regex i sapplied
    console.log(newText);
    const encodedInputText = encodeURI(input.value);
    const encodedCleanText = encodeURI(newText);
    console.log(encodedInputText, encodedCleanText);
})